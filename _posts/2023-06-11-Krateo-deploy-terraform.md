...
Krateo isn't in the Terraform state either, only the `null_resource` representing it -- so if we do a `terraform destroy`, the resources that Terraform knows about will be destroyed, but _not those Krateo made_.
